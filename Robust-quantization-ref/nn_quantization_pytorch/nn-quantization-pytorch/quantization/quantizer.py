...
class ModelQuantizer:
    ...
    def get_quantizer_state(self):
        """
        Retrieve the state of all quantization wrappers and ensure it is JSON serializable.
        """
        quantizer_states = {}
        if self.bit_weights is not None or self.bit_act is not None:
            with torch.no_grad():
                for name, qwrapper in self.quantization_wrappers:
                    if hasattr(qwrapper, "state_dict"):
                        state = qwrapper.state_dict()
                        # Convert tensors to lists
                        quantizer_states[name] = {k: v.tolist() if isinstance(v, torch.Tensor) else v for k, v in state.items()}
                    else:
                        quantizer_states[name] = {}
