    ...
    for key, value in state_dict.items():
        if 'weight' in key:  # Process only weight tensors
            ...
            if alpha_key in state_dict:  # If alpha exists
                ...
                dequantized_state_dict[key] = value * alpha  # Dequantize weight
            else:
...
def asymmetric_quantize(x, num_bits, x_min=None, x_max=None):
    """
    ...
    - x_min (float, optional): Minimum value of the input range. If None, inferred from x.
    - x_max (float, optional): Maximum value of the input range. If None, inferred from x.
    ...
    # Infer min and max if not provided
    if x_min is None:
        x_min = x.min().item()
    if x_max is None:
        x_max = x.max().item()
...
def asymmetric_dequantize(x_quantized, scale, zero_point):
    """
    Dequantization function for asymmetric quantization.
...
def feedforward_asymmetric(x, model_dictionary, num_bits_weight, num_bits_bias, num_bits_activation):
    """
    Perform feedforward inference using asymmetric quantization.

    Args:
    - x (torch.Tensor): Input tensor.
    - model_dictionary (dict): Dictionary of model parameters (weights and biases).
    ...
    # Quantize activations (input layer)
    ...
    for i in ['1', '2', '3', '4', '5']:
        ...
        weight_quantized, w_scale, w_zero_point = asymmetric_quantize(model_dictionary[name + 'weight'], num_bits_weight)
        bias_quantized, b_scale, b_zero_point = asymmetric_quantize(model_dictionary[name + 'bias'], num_bits_bias)
...
if __name__ == '__main__':
    ...
    path = f'quantization_results/CIFAR10/bound_quantization_error_norm_1_assym_act_8_qat.xlsx'
