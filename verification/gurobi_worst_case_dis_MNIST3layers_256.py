...
def test_robustness(model_dictionary, net, testloader, epsilon_input=1/255, epsilon_weight=2/255, epsilon_bias=2/255, epsilon_activation=2/255):
    ...
    for batch_idx,  (inputs, labels)  in enumerate(testloader, 0):
        ...
        np.save('opt_results/exp3_min_logs', min_outputs) 
        np.save('opt_results/exp3_max_logs',max_outputs)
        np.save( 'opt_results/exp3_best_logs', best_list) 
        np.save( 'opt_results/exp3_worst_logs', worst_list) 
        total += 1
        end_time = time.time()
