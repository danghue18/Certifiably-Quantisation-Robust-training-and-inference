...
root = os.getenv('ROOT')
checkpoint_path = os.path.join(root, os.getenv("CHECKPOINT_PATH18"))
weight_folder = os.path.join(root,os.getenv('WEIGHT_FOLDER18'))
...
def test_robustness(model_dictionary, net, testloader, epsilon_input=1/255, epsilon_weight=2/255, epsilon_bias=2/255, epsilon_activation=2/255, timeout=600):
    ...
    for batch_idx,  (inputs, labels)  in enumerate(testloader, 0):
        ...
        if not optimization_status:  
            ...
            path = f'opt_results/exp18_{epsilon_input}_{epsilon_weight}_{epsilon_activation}.xlsx'
...
if __name__ == '__main__':
    #8 bits
    test_robustness(model_dictionary,net, testloader, epsilon_input=1/256, epsilon_weight=1/128, epsilon_bias=1/128, epsilon_activation=1/128, timeout=1800)