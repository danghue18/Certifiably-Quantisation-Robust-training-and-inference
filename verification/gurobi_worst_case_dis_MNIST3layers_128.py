...
def test_robustness(model_dictionary, net, testloader, epsilon_input=1/255, epsilon_weight=2/255, epsilon_bias=2/255, epsilon_activation=2/255):
    ...
    for batch_idx,  (inputs, labels)  in enumerate(testloader, 0):
        ...
        #     worst_case_dis = diff
        # result = {'Best ': best, 'Worst ': worst, 'original output': orig_output}
        # path = f'opt_results/exp2_sample{batch_idx+1}.xlsx'
        # DictExcelSaver.save(result,path)
        min_outputs.append(min_output)
        max_outputs.append(max_output)
        best_list.append(best)
        worst_list.append(worst)
        np.save('opt_results/exp2_min_logs', min_outputs) 
        np.save('opt_results/exp2_max_logs',max_outputs)
        np.save( 'opt_results/exp2_best_logs', best_list) 
        np.save( 'opt_results/exp2_worst_logs', worst_list) 
        total += 1
        end_time = time.time()
