...
root = os.getenv('ROOT')
checkpoint_path = os.path.join(root, os.getenv("CHECKPOINT_PATH2"))
weight_folder = os.path.join(root,os.getenv('WEIGHT_FOLDER2'))
...
def test_robustness(model_dictionary, net, testloader, epsilon_input=1/255, epsilon_weight=2/255, epsilon_bias=2/255, epsilon_activation=2/255):
    ...
    for batch_idx,  (inputs, labels)  in enumerate(testloader, 0):
        ...
        if device == 'cuda': 
            ...
            bounds = net.linear_bound(torch.cat([x_ub, x_lb], 0), epsilon_weight, epsilon_bias, epsilon_activation)
        # check bounds
        #print(len(bounds))
        ...
        print(f"*************Sample {batch_idx+1} has worst-case dis: {diff}")
        print(f"*************Worst case discrepancy with ep_i = {epsilon_input}, ep_w = {epsilon_weight}, ep_b = {epsilon_bias},ep_a = {epsilon_activation} over {batch_idx+1} samples: {worst_case_dis}")
        result = {'ep_i':epsilon_input, 'ep_w': epsilon_weight, 'ep_b': epsilon_bias, 'ep_a':epsilon_activation, 'number of samples': batch_idx+1, 'Worst case discrepancy': worst_case_dis }
        path = f'opt_results/exp2.xlsx'
