...
root = os.getenv('ROOT')
checkpoint_path = os.path.join(root, os.getenv("CHECKPOINT_PATH10"))
weight_folder = os.path.join(root,os.getenv('WEIGHT_FOLDER10'))
...
def test_robustness(model_dictionary, net, testloader, epsilon_input=1/255, epsilon_weight=2/255, epsilon_bias=2/255, epsilon_activation=2/255):
    ...
    for batch_idx,  (inputs, labels)  in enumerate(testloader, 0):
        ...
        weight5 = model_gurobi.addVars(10, n_hidden_nodes, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="weight5")
        bias5 = model_gurobi.addVars(10, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="bias5")
        ...
        for i in range(10):
            model_gurobi.addConstr(output[i] == gp.quicksum([weight5[i, j] * x5[j] for j in range(n_hidden_nodes)]) + bias5[i])
            model_gurobi.addConstr(output[i] >= lower_bound5[0,i].item())
            # print("----Output lower bound:", lower_bound3[0,i].item())
            # print("----Output upper bound:", upper_bound3[0,i].item())
            model_gurobi.addConstr(output[i] <= upper_bound5[0,i].item())
        ...
        path = f'opt_results/exp10_sample{batch_idx+1}.xlsx'
        DictExcelSaver.save(result,path)
        total += 1
        end_time = time.time()
